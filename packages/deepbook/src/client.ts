...
import {
	...
	MODULE_CLOB,
	MODULE_CUSTODIAN,
	NORMALIZED_MYS_COIN_TYPE,
	ORDER_DEFAULT_EXPIRATION_IN_MS,
	PACKAGE_ID,
} from './utils/index.js';

const DUMMY_ADDRESS = normalizeMysAddress('0x0');

export class DeepBookClient {
	#poolTypeArgsCache: Map<string, string[]> = new Map();
	/**
	 *
	 * @param mysClient connection to fullnode
	 ...
	 */
	constructor(
		public mysClient: MysClient = new MysClient({ url: getFullnodeUrl('testnet') }),
	 ...
	 * @param coinId the coin used for the deposit. You can omit this argument if you are depositing MYS, in which case
	...
	async deposit(
		...
		const hasMys =
			baseAsset === NORMALIZED_MYS_COIN_TYPE || quoteAsset === NORMALIZED_MYS_COIN_TYPE;

		if (coinId === undefined && !hasMys) {
			throw new Error('coinId must be specified if neither baseAsset nor quoteAsset is MYS');
		}
		...
		const coinType = coinId ? await this.getCoinType(coinId) : NORMALIZED_MYS_COIN_TYPE;
	...
	async getAllPools(
		...
		const resp = await this.mysClient.queryEvents({
	...
	async getPoolInfo(poolId: string): Promise<PoolSummary> {
		const resp = await this.mysClient.getObject({
			id: poolId,
	...
	async getOrderStatus(
		...
		const results = (
			await this.mysClient.devInspectTransactionBlock({
				transactionBlock: tx,
	...
	async getUserPosition(
		...
		tx.moveCall({
			...
			arguments: [tx.object(normalizeMysObjectId(poolId)), tx.object(cap)],
		});
	...
	async getLevel2BookStatus(
		...
		const results = await this.mysClient.devInspectTransactionBlock({
			transactionBlock: tx,
			sender: this.currentAddress,
		});
	...
	#checkAccountCap(accountCap: string | undefined = undefined): string {
		...
		return normalizeMysObjectId(cap);
	}
	...
	#checkAddress(recipientAddress: string): string {
		...
		return normalizeMysAddress(recipientAddress);
	}
	...
	public async getCoinType(coinId: string) {
		...
		if (
			parsed?.address === NORMALIZED_MYS_COIN_TYPE.split('::')[0] &&
			parsed.module === 'coin' &&
			parsed.name === 'Coin' &&
