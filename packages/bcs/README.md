...
```ts
// Example: Generics
import { bcs, BcsType } from '@socialproof/bcs';
...
```ts
import { bcs, toHex } from '@socialproof/bcs';
...
transforms can also handle more complex types. For instance, `@socialproof/mys` uses the following
...
```ts
import { bcs, fromBase58, fromBase64, fromHex } from '@socialproof/bcs';
...
```ts
import { bcs, type InferBcsType, type InferBcsInput } from '@socialproof/bcs';
