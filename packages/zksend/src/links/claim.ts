...
const MYS_COIN_OBJECT_TYPE = normalizeStructTag('0x2::coin::Coin<0x2::mys::MYS>');
...
export class ZkSendLink {
	...
	constructor({
		...
		this.address = address ?? keypair!.toMysAddress();
	...
	async claimAssets(
		...
		const sponsored = await this.#createSponsoredTransaction(
			tx,
			address,
			reclaim ? address : this.keypair!.toMysAddress(),
		);
	...
	createClaimTransaction(
		...
		const sender = reclaim ? address : this.keypair!.toMysAddress();
		tx.setSender(sender);
	...
	async createRegenerateTransaction(
		...
		const to = tx.pure.address(newLinkKp.toMysAddress());
	...
	async #loadClaimedAssets() {
		...
		const transfer = tx.transaction.data.transaction.transactions.findLast(
			(tx): tx is Extract<MysTransaction, { TransferObjects: unknown }> => 'TransferObjects' in tx,
		);
	...
	async #listNonContractClaimableAssets() {
		...
		if (this.#ownedObjects.length === 0 && !this.#hasMys) {
			return {
				balances,
				nfts,
				coins,
	...
	#createNonContractClaimTransaction(address: string) {
		...
		const tx = new Transaction();
		tx.setSender(this.keypair.toMysAddress());
		...
		const objectsToTransfer: TransactionObjectArgument[] = this.#ownedObjects
			.filter((object) => {
				...
				} else if (object.type === MYS_COIN_OBJECT_TYPE) {
					return false;
				}
	...
	async #loadOwnedObjects({
			...
			for (const object of ownedObjects.data) {
				if (object.data) {
					this.#ownedObjects.push({
						objectId: normalizeMysObjectId(object.data.objectId),
						version: object.data.version,
						digest: object.data.digest,
		...
		const coins = await this.#client.getCoins({
			coinType: MYS_COIN_TYPE,
			owner: this.address,
		});

		this.#hasMys = coins.data.length > 0;
		...
		if (this.#hasMys || this.#ownedObjects.length > 0) {
			this.claimed = false;
