...
The `@socialproof/sui/bcs` package extends `@socialproof/bcs` with Sui specific scheme definitions.

To learn more about using BCS see the [BCS documentation](/bcs).

the `bcs` export of `@socialproof/sui/bcs` contains all the same exports as `bcs` from `@socialproof/bcs` plus
