...
import { normalizeMysAddress } from '../../../src/utils/mys-types.js';
...
describe('Publickey', () => {
	...
	it('`toMysAddress()` should return correct mys address associated with multisig publickey', async () => {
		const multiSigPublicKey = MultiSigPublicKey.fromPublicKeys({
			threshold: 3,
			publicKeys: [
				{ publicKey: pk1, weight: 1 },
		...
		for (const { publicKey, weight } of multiSigPublicKey.getPublicKeys()) {
			const bytes = publicKey.toMysBytes();
			tmp.set(bytes, i);
			i += bytes.length;
			tmp.set([weight], i++);
		}
		const multisigMysAddress = normalizeMysAddress(
			bytesToHex(blake2b(tmp.slice(0, i), { dkLen: 32 })),
		);

		expect(multiSigPublicKey.toMysAddress()).toEqual(multisigMysAddress);
		expect(multiSigPublicKey.toMysAddress()).toEqual(
