...
const TEST_CASES = [
	[
		...
		'mysprivkey1qgj6vet4rstf2p00j860xctkg4fyqqq5hxgu4mm0eg60fq787ujnqs5wc8q',
	...
	[
		...
		'mysprivkey1qgmgr6dza8slgxn0rcxcy47xeas9l565cc5q440ngdzr575rc2356gzlq7a',
	...
	[
		...
		'mysprivkey1qt2gsye4dyn0lxey0ht6d5f2ada7ew9044a49y2f3mymy2uf0hr55jmfze3',
...
describe('secp256r1-keypair', () => {
	...
	it('incorrect purpose node for secp256r1 derivation path', () => {
		expect(() => {
			Secp256r1Keypair.deriveKeypair(TEST_CASES[0][0], `m/54'/6976'/0'/0'/0'`);
		}).toThrow('Invalid derivation path');
	});

	it('incorrect hardened path for secp256k1 key derivation', () => {
		expect(() => {
			Secp256r1Keypair.deriveKeypair(TEST_CASES[0][0], `m/44'/6976'/0'/0'/0'`);
		}).toThrow('Invalid derivation path');
	});
