...
import { fromBase64, toBase58, toBase64 } from '@mysocial/bcs';
...
describe('Multisig scenarios', () => {
	it('multisig address creation and combine sigs using Secp256r1Keypair', async () => {
		...
		tx.setSender(k3.getPublicKey().toMysAddress());
		tx.setGasPrice(5);
		tx.setGasBudget(100);
		tx.setGasPayment([
			{
...
describe('Multisig address creation:', () => {
	...
	it('`toMultiSigAddress()` with zklogin identifiers', async () => {
		...
		const multisigAddress = multisigPublicKey.toMysAddress();
...
describe('MultisigKeypair', () => {
	test('signTransaction', async () => {
		...
		expect(signerPubKey.toMysAddress()).toEqual(publicKey.toMysAddress());
		...
		expect(signerPubKey2.toMysAddress()).toEqual(publicKey.toMysAddress());
	...
	test('unknown signers', async () => {
		...
		expect(() => publicKey.getSigner(k2)).toThrow(
			new Error(`Signer ${pk2.toMysAddress()} is not part of the MultiSig public key`),
		);
	});
});
