...
import { normalizeMysAddress, MYS_ADDRESS_LENGTH } from '../../../src/utils/mys-types.js';
...
describe('Publickey', () => {
	...
	it('`toMysPublicKey()` should return a valid mys representation', async () => {
		expect(pk2.toMysPublicKey()).toEqual(toBase64(pk2.toMysBytes()));
		expect(pk2.toMysPublicKey()).toEqual('AQIdFSMHxrcrDtBBiw5wzYDn9SlbjYb1ci0/UhP70jlPNg==');
	});
	...
	it('`toMysBytes()` should return the correct byte representation of the public key with the signature scheme flag', async () => {
		const pk1MysBytes = new Uint8Array(pk1.toRawBytes().length + 1);
		pk1MysBytes.set([0x00]);
		pk1MysBytes.set(pk1.toRawBytes(), 1);

		expect(pk1.toMysBytes()).toEqual(pk1MysBytes);
		expect(pk1.toMysBytes()).toEqual(
			new Uint8Array([
		...
		const pk2MysBytes = new Uint8Array(pk2.toRawBytes().length + 1);
		pk2MysBytes.set([0x01]);
		pk2MysBytes.set(pk2.toRawBytes(), 1);

		expect(pk2.toMysBytes()).toEqual(pk2MysBytes);
		expect(pk2.toMysBytes()).toEqual(
			new Uint8Array([
		...
		const pk3MysBytes = new Uint8Array(pk3.toRawBytes().length + 1);
		pk3MysBytes.set([0x02]);
		pk3MysBytes.set(pk3.toRawBytes(), 1);

		expect(pk3.toMysBytes()).toEqual(pk3MysBytes);
		expect(pk3.toMysBytes()).toEqual(
			new Uint8Array([
	...
	it('`toMysAddress()` should correctly return mys address associated with Ed25519 publickey', async () => {
		const pk1MysAddress = normalizeMysAddress(
			bytesToHex(blake2b(pk1.toMysBytes(), { dkLen: 32 })).slice(0, MYS_ADDRESS_LENGTH * 2),
		);
		const pk2MysAddress = normalizeMysAddress(
			bytesToHex(blake2b(pk2.toMysBytes(), { dkLen: 32 })).slice(0, MYS_ADDRESS_LENGTH * 2),
		);
		const pk3MysAddress = normalizeMysAddress(
			bytesToHex(blake2b(pk3.toMysBytes(), { dkLen: 32 })).slice(0, MYS_ADDRESS_LENGTH * 2),
		);
		expect(k1.toMysAddress()).toEqual(pk1MysAddress);
		expect(k1.toMysAddress()).toEqual(
			'0xafedf3bc60bd296aa6830d7c48ca44e0f7a32478ae4bd7b9a6ac1dc81ff7b29b',
		);

		expect(k2.toMysAddress()).toEqual(pk2MysAddress);
		expect(k2.toMysAddress()).toEqual(
			'0x7e4f9a35bf3b5383802d990956d6f3c93e6184ebbbcf0820c124ab3a59ef77ac',
		);

		expect(k3.toMysAddress()).toEqual(pk3MysAddress);
		expect(k3.toMysAddress()).toEqual(
